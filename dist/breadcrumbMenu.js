...
exports.addBlockBdMenuListener = addBlockBdMenuListener;
/**
...
async function addBlockBdMenuListener(protyle) {
    var _a;
    // 限制范围，避免影响插件插入的面包屑
    const breadcrumbBar = (_a = protyle === null || protyle === void 0 ? void 0 : protyle.breadcrumb) === null || _a === void 0 ? void 0 : _a.element;
    ...
    breadcrumbBar.addEventListener('click', async (event) => {
        ...
        if (!precedingItem || !precedingItem.classList.contains('protyle-breadcrumb__item')) {
            (0, logger_1.logPush)("未找到箭头左侧的面包屑项目。");
            return;
        }
        ...
        if (!nodeId || !iconUseElement) {
            (0, logger_1.logPush)("无法从面包屑项目中提取 node-id 或 icon。");
            return;
        }
        ...
        (0, logger_1.logPush)(`点击了 ID: ${nodeId} (${iconHref}) 旁边的箭头`);
        const menuId = "bid_" + nodeId;
        if (!(0, menus_1.checkAndCloseLastMenu)(menuId)) {
            return;
        }
        try {
            // 获取文档大纲
            const outlineData = await (0, api_1.getDocOutline)(docId);
            let menuItems = [];
            if (outlineData == null) {
                (0, logger_1.logPush)("获取大纲数据失败或文档无大纲。");
                (0, siyuan_1.showMessage)(state_1.state.language["nothingToDisplay"] + "--- fakeDocBreadcrumb");
                return;
            }
            // 根据图标类型来决定菜单内容
            if (iconHref === '#iconFile') {
                // 如果是文档图标，显示所有顶级标题
                (0, logger_1.logPush)("目标是文档根节点，筛选所有顶级标题 (depth: 0)...");
                menuItems = outlineData.filter(item => item.depth === 0);
            }
            else if (iconHref.startsWith('#iconH')) {
                // 如果是标题图标 (H1-H6)，显示其下的直接子标题
                (0, logger_1.logPush)(`目标是标题节点，查找 ID: ${nodeId} 的子标题...`);
                // 递归查找指定 ID 的标题及其子项
                ...
                if (parentHeading) {
                    ...
                    (0, logger_1.logPush)(`标题 ${nodeId} 没有找到或没有子标题。`);
                }
            }
            else {
                (0, logger_1.logPush)(`点击了非文档或标题图标 (${iconHref}) 旁的箭头，不作处理。`);
                (0, siyuan_1.showMessage)(state_1.state.language["nothingToDisplay"] + "--- fakeDocBreadcrumb");
                return;
            }
            // 递归构建菜单项的函数
            function buildMenuItems(items) {
                return items.map(item => {
                    const fullName = (0, utils_1.escapeHTML)((0, utils_1.stripHTML)(item.name || item.content || "N/A"));
                    const trimedName = fullName.length > state_1.state.g_setting.nameMaxLength ? fullName.substring(0, state_1.state.g_setting.nameMaxLength) + "..." : fullName;
                    const menuItem = {
                        id: item.id,
                        ...
                        click: (htmlElement, event) => {
                            var _a;
                            const blocId = (_a = htmlElement.querySelector(".og-fake-doc-breadcrumb-menu-item-container")) === null || _a === void 0 ? void 0 : _a.getAttribute("data-og-block-node-id");
                            event.preventDefault();
                            event.stopImmediatePropagation();
                            event.stopPropagation();
                            if (blocId) {
                                (0, siyuan_1.openTab)({
                                    ...
                                    afterOpen: () => {
                                        var _a;
                                        // 更新breadcrumb
                                        (_a = protyle === null || protyle === void 0 ? void 0 : protyle.breadcrumb) === null || _a === void 0 ? void 0 : _a.render(protyle);
                                    }
                                });
                            }
                        }
            ...
            if (menuItems.length > 0) {
                ...
                (0, menus_1.saveLastMenu)(tempMenu, menuId);
            }
            else {
                (0, logger_1.logPush)("没有可供显示的菜单项。");
                (0, siyuan_1.showMessage)(state_1.state.language["nothingToDisplay"] + "--- fakeDocBreadcrumb");
            }
        }
        catch (error) {
            (0, logger_1.errorPush)("获取或处理大纲数据时出错:", error);
        }
    });
}
