...
export class SettingProperty {
     ...
     * @param {*} limit 限制
     */
    constructor(id: string, type: string, limit: any, value: any = undefined) {
        this.id = `${CONSTANTS.PLUGIN_NAME}_${id}`;
        this.simpId = id;
        this.name = state.language[`setting_${id}_name`] ?? id;
        this.desp = state.language[`setting_${id}_desp`] ?? id + "_desp";
        this.type = type;
        this.limit = limit;
        if (value) {
            this.value = value;
        } else {
            this.value = state.g_setting[this.simpId];
        }
...
export function generateSettingPanel(settingObjectArray: any[]) {
    ...
    for (const oneSettingProperty of settingObjectArray) {
        ...
        if (oneSettingProperty.name.includes("🧪")) {
            const experimentalText = state.language["setting_experimental"] || "（实验性功能）";
        ...
        switch (oneSettingProperty.type) {
            case "NUMBER": {
                ...
                (controlElement as HTMLInputElement).type = "number";
                const [min, max] = oneSettingProperty.limit || [null, null];
                if (min !== null) (controlElement as HTMLInputElement).min = min;
                if (max !== null) (controlElement as HTMLInputElement).max = max;
                (controlElement as HTMLInputElement).value = oneSettingProperty.value;
                break;
            }
            case "SELECT": {
                ...
                oneSettingProperty.limit.forEach((option: any) => {
                    const optionElement = document.createElement("option");
                    optionElement.value = option.value;
                    let optionName = state.language[`setting_${oneSettingProperty.simpId}_option_${option.value}`] || option.value;
            ...
            case "TEXT": {
                controlElement = document.createElement("input");
                controlElement.className = "b3-text-field fn__flex-center fn__size200";
                (controlElement as HTMLInputElement).type = "text";
                (controlElement as HTMLInputElement).value = oneSettingProperty.value;
                break;
            }
            case "SWITCH": {
                ...
                (controlElement as HTMLInputElement).type = "checkbox";
                (controlElement as HTMLInputElement).checked = !!oneSettingProperty.value;
                break;
            }
            case "TEXTAREA": {
                ...
                (controlElement as HTMLTextAreaElement).value = oneSettingProperty.value;
        ...
        if (controlElement) {
            ...
            if (oneSettingProperty.simpId) (controlElement as any).name = oneSettingProperty.simpId;
 ...
 * 由设置界面读取配置
 */
export function loadUISettings(formElement: HTMLFormElement) {
    ...
    for (let number of numbers) {
        let numberElement = number as HTMLInputElement;
